...
7. `docs/wiki/index.md`, `current-status.md`, `log.md` (last 10 entries)
8. `pmo/dashboard.md`
9. `docs/raw/` — all raw requirement docs (`manifests/raw_sources.csv` for the index)
10. Topic-specific wiki pages relevant to the current task
...
- `pmo/stories/STORY-{NNN}-{slug}.md` — user stories
...
### When asked to write stories for a module
1. Re-read `docs/wiki/module-{name}.md` and relevant personas
2. Slice the module into stories — each story:
...
3. Write `pmo/stories/STORY-{NNN}-{slug}.md` for each
4. Update `pmo/dashboard.md` with the new stories (status: 📝 Drafted)
5. Hand off to Architect via `pmo/handoffs/HANDOFF-NNN-pm-to-architect-{topic}.md`
